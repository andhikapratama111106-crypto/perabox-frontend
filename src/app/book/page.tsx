...
Mohon konfirmasinya. Terima kasih.`;
            ...
            const centralNumber = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '81234567894';
    ...
    return (
        <main className="min-h-screen bg-gray-50 pb-20">
            ...
            <div className="container mx-auto px-4 mt-8 max-w-5xl overflow-hidden">
                                        ...
                                        if (displayTechs.length === 0 && !loading) {
                                            return (
                                                <div className="col-span-full text-center py-10 bg-white rounded-3xl border-2 border-dashed border-gray-200">
                                                    ...
                                                    <button onClick={() => window.location.reload()} className="bg-primary/10 text-primary px-6 py-2 rounded-full font-bold hover:bg-primary/20 transition-all">
                                                        {t('bookPage.tryAgain')}
                                                    </button>
                                                </div>
                                            );
                                        ...
                                        return (
                                            <>
                                                {displayTechs.map((tech: Technician, index: number) => (
                                                    <TechnicianCard key={tech.id} technician={tech} onSelect={handleTechnicianSelect} priority={index < 4} />
                                                ))}
                                            </>
                                        );
                                    })()}
                        ...
                        >
                            <div className="max-w-2xl mx-auto bg-white rounded-3xl shadow-sm p-6 md:p-8">
                                ...
                                <div className="mb-6">
                                    ...
                                    <input type="date" className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all" value={selectedDate} onChange={handleDateChange} min={todayStr} max="2028-12-31" />
                                </div>
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                        {(() => {
                                            ...
                                            return timeSlots.map((time: string) => {
                                                ...
                                                return (
                                                    <button key={time} type="button" disabled={isPast} onClick={() => setSelectedTime(time)} className={`py-3 rounded-2xl font-bold transition-all border-2 ${selectedTime === time ? 'bg-primary border-primary text-white shadow-lg shadow-primary/20 scale-[0.98]' : isPast ? 'bg-gray-50 border-gray-100 text-gray-300 cursor-not-allowed' : 'bg-white border-gray-100 text-dark hover:border-primary/30 hover:bg-primary/5'}`}>
                                                        {time}
                                                    </button>
                                                );
                                            });
                                ...
                                <div className="mt-8 flex justify-between">
                                    ...
                                    <button onClick={handleScheduleSelect} disabled={!selectedDate || !selectedTime} className="bg-primary hover:bg-primary/90 disabled:opacity-50 text-white px-8 py-3 rounded-full font-bold transition-all">
                            ...
                            <div className="max-w-2xl mx-auto bg-white rounded-3xl shadow-sm p-6 md:p-8">
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {apiServices.length > 0 ? apiServices.map((service) => (
                                        <div key={service.id} onClick={() => handleServiceToggle(service.id)} className={`group relative flex items-center p-6 rounded-[2rem] border-2 cursor-pointer transition-all duration-300 ${selectedServices.includes(service.id) ? 'border-[#9C6D3F] bg-[#FDF8F3] shadow-lg scale-[1.01]' : 'border-gray-100 bg-white hover:border-[#9C6D3F]/30 hover:shadow-md'}`}>
                                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl mr-6 transition-all ${selectedServices.includes(service.id) ? 'bg-[#9C6D3F] text-white shadow-lg rotate-12' : 'bg-gray-50 text-gray-400 group-hover:bg-amber-50'}`}>
                                            ...
                                            <div className={`w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all ${selectedServices.includes(service.id) ? 'bg-[#9C6D3F] border-[#9C6D3F] shadow-inner' : 'border-gray-200 group-hover:border-[#9C6D3F]/50'}`}>
                                    ...
                                    )) : <div className="text-center py-20 bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200"><p className="text-gray-400 font-medium">{t('bookPage.loadingServices')}</p></div>}
                                </div>
                                <div className="mt-8 flex justify-between">
                            ...
                            <div className="max-w-2xl mx-auto bg-white rounded-3xl shadow-sm p-6 md:p-8">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="tel" className={`w-full px-4 py-3 rounded-xl border outline-none focus:ring-2 focus:ring-primary/20 transition-all ${formData.phone && (!formData.phone.startsWith('08') || !/^\d+$/.test(formData.phone)) ? 'border-red-400 bg-red-50' : 'border-gray-200 focus:border-primary'}`} value={formData.phone} onChange={(e: any) => { const val = e.target.value.replace(/[^\d]/g, ''); if (val.length <= 13) setFormData({ ...formData, phone: val }); }} placeholder={t('bookPage.whatsappPlaceholder')} />
                                        {formData.phone && !formData.phone.startsWith('08') && <p className="text-red-500 text-xs mt-1 font-bold">{t('bookPage.whatsappErr1')}</p>}
                                        {formData.phone && formData.phone.startsWith('08') && (formData.phone.length < 10 || formData.phone.length > 12) && <p className="text-red-500 text-xs mt-1 font-bold">{t('bookPage.whatsappErr2')}</p>}
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('bookPage.address')}</label>
                                        <MapPicker onAddressSelect={(addr: string) => setFormData((prev: any) => ({ ...prev, address: addr }))} initialAddress={formData.address} />
                                        <div className="mt-4">
                                            <textarea className="w-full px-4 py-3 rounded-xl border border-gray-200 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 min-h-[80px] text-sm" value={formData.address} onChange={(e: any) => setFormData({ ...formData, address: e.target.value })} placeholder={t('bookPage.addressPlaceholder')} />
                                        </div>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('bookPage.notes')}</label>
                                        <input type="text" className="w-full px-4 py-3 rounded-xl border border-gray-200 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" value={formData.notes} onChange={(e: any) => setFormData({ ...formData, notes: e.target.value })} placeholder={t('bookPage.notesPlaceholder')} />
                                    </div>
                                </div>
                                <div className="mt-8 flex justify-between">
                        ...
                        >
                            <div className="max-w-2xl mx-auto bg-white rounded-3xl shadow-sm p-6 md:p-8">
                                ...
                                <div className="mt-8 flex justify-between items-center">
                                    ...
                                    <button onClick={handleCreateBooking} disabled={submitting || !paymentMethod} className="bg-primary hover:bg-primary/90 disabled:opacity-50 text-white px-8 py-3 rounded-full font-bold transition-all shadow-lg hover:shadow-primary/30">{submitting ? t('bookPage.btnProcess') : t('bookPage.btnConfirm')}</button>
                                </div>
                            </div>
                        </motion.div>
                    )}
